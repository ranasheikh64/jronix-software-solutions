...
const jobs = [
  { title: "Flutter Developer", type: "Full-time", location: "Remote", dept: "Engineering" },
  { title: "React Frontend Engineer", type: "Full-time", location: "Dhaka / Remote", dept: "Engineering" },
  { title: "UI/UX Designer", type: "Part-time", location: "Remote", dept: "Design" },
  { title: "AI/ML Engineer", type: "Full-time", location: "Remote", dept: "AI" },
  { title: "WordPress Developer", type: "Contract", location: "Remote", dept: "Web" },
];

function JobApplyModal({ job, onClose }: { job: typeof jobs[0]; onClose: () => void }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      ...
      >
        ...
        <div className="flex flex-col gap-3">
          {["Full Name", "Email Address", "Phone Number"].map((label) => (
            <input key={label} placeholder={label} className="w-full px-4 py-2.5 rounded-lg text-sm outline-none" style={{ background: "#0d1f3c", border: "1px solid rgba(0,170,255,0.15)", color: "#e8f4ff", fontFamily: "Inter, sans-serif" }} />
          ))}
          <textarea placeholder="Cover Letter (optional)" rows={3} className="w-full px-4 py-2.5 rounded-lg text-sm outline-none resize-none" style={{ background: "#0d1f3c", border: "1px solid rgba(0,170,255,0.15)", color: "#e8f4ff", fontFamily: "Inter, sans-serif" }} />
          <div className="flex gap-3 mt-2">
            <button className="flex-1 py-2.5 rounded-lg text-sm" style={{ background: "linear-gradient(135deg, #0077cc, #00d4ff)", color: "#fff", fontFamily: "Inter, sans-serif" }}>Submit Application</button>
            <button onClick={onClose} className="px-5 py-2.5 rounded-lg text-sm" style={{ border: "1px solid rgba(0,170,255,0.2)", color: "#7aa8cc", fontFamily: "Inter, sans-serif" }}>Cancel</button>
          </div>
        </div>
      </motion.div>
    </motion.div>
